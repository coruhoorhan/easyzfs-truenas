...
var catalogo = map[string]map[string]textos{
	"es": {
		...
		"broken_chain":       {"Cadena de respaldo rota", "{machine}: falta el VBK principal (cadena {chain}), no se puede restablecer desde los VIB posteriores."},
		"stale_backup":       {"Respaldo obsoleto", "{machine}: último respaldo el {last}, hace {days} días."},
		"chain_gap":          {"Hueco en la cadena", "{machine}: {days} días sin respaldo en una cadena (datos no restaurables en ese intervalo)."},
	...
	"en": {
		...
		"broken_chain":       {"Broken backup chain", "{machine}: VBK missing (chain {chain}), cannot restore from later VIBs."},
		"stale_backup":       {"Stale backup", "{machine}: last backup {last}, {days} days ago."},
		"chain_gap":          {"Gap in backup chain", "{machine}: {days} days without backup in a chain (data in that interval not restorable)."},
